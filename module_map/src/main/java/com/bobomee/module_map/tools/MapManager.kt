package com.bobomee.module_map.tools
...
import com.bobomee.module_map.R
...
object MapManager {
    ...
    //初始化
    fun initMapManager(mContext: Context) {
        MapManager.mContext = mContext
    ...
    fun setLocationSwitch(isOpen: Boolean, mOnLocationResultListener: OnLocationResultListener?) {
        if (isOpen) {
            MapManager.mOnLocationResultListener = mOnLocationResultListener
        ...
        mLocationClient.registerLocationListener(object : BDAbstractLocationListener() {
            override fun onReceiveLocation(location: BDLocation?) {
                ...
                if (location.locType == 61 || location.locType == 161) {
                    ...
                    LogUtils.i("==>定位失败原因：" + location.locType)
    ...
    private fun poi(keyword: String, city: String,size: Int) {
        LogUtils.i("keyword:$keyword" + "city:$city")
    ...
    fun poiSearch(keyword: String, city: String, size:Int,mOnPoiResultListener: OnPoiResultListener?) {
        MapManager.mOnPoiResultListener = mOnPoiResultListener
    ...
    fun startLocationWalkingSearch(toAddress: String, mOnNaviResultListener: OnNaviResultListener) {
        MapManager.mOnNaviResultListener = mOnNaviResultListener
        //定位
        setLocationSwitch(true, object : OnLocationResultListener {
            override fun result(
                la: Double,
    ...
    fun initNaviEngine(
        ...
        WalkNavigateHelper.getInstance().initNaviEngine(mActivity, object : IWEngineInitListener {
            override fun engineInitSuccess() {
                ...
                LogUtils.i( "导航引擎初始化成功")
            }

            override fun engineInitFail() {
                //引擎初始化失败的回调
                LogUtils.e("导航引擎初始化失败")
            }
        })
    }
    ...
    //截图
    fun snapshot() {
        mBaiduMap?.snapshot { LogUtils.i("===>截图完成") }
    }
    ...
    private fun initListener() {
        ...
        //双击
        mBaiduMap?.setOnMapDoubleClickListener(BaiduMap.OnMapDoubleClickListener { LogUtils.i("双击") })

        //长按
        mBaiduMap?.setOnMapLongClickListener { LogUtils.i("长按") }
    }
            ...
            override fun onGetGeoCodeResult(geoCodeResult: GeoCodeResult?) {
                ...
                if (null != geoCodeResult && null != geoCodeResult.location) {
                    if (geoCodeResult.error !== SearchResult.ERRORNO.NO_ERROR) {
                        ...
                        LogUtils.i( "正编码成功：$latitude:$longitude")
    ...
    fun startCode(city: String, address: String, mOnCodeResultListener: OnCodeResultListener) {
        MapManager.mOnCodeResultListener = mOnCodeResultListener
        mCoder.geocode(
            GeoCodeOption()
                .city(city)
                .address(address)
